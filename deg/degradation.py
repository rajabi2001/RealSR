...
if __name__ == '__main__':
    ...
    prepro = SRMDPreprocessing(
        scale=4, pca_matrix=pca_matrix,
        ksize=31, code_length=15,
        random_kernel=True, noise=True, cuda=True, random_disturb=False,
        sig=0, sig_min=0.2, sig_max=4.0, rate_iso=0.0, rate_cln=0.0, noise_high=20/255,
        stored_kernel=False, pre_kernel_path=None
    )
    ...
    # perform degradation
    img_lr, blur_pca = prepro(img_hr, kernel=False)

    # plot
    print(blur_pca.shape)
    plt.imshow(img_lr[0].permute(1, 2, 0).cpu().numpy())
    plt.show()
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, feature_layer=[2,7,16,25,34], weights=[0.1,0.1,1.0,1.0,1.0], lossfn_type='l1', use_input_norm=True, use_range_norm=False):
        ...
        self.vgg = VGGFeatureExtractor(feature_layer=feature_layer, use_input_norm=use_input_norm, use_range_norm=use_range_norm).cuda()
        self.lossfn_type = lossfn_type
        self.weights = weights
        if self.lossfn_type == 'l1':
            self.lossfn = nn.L1Loss()
        elif self.lossfn_type == 'cosinesim':
            self.lossfn = nn.CosineSimilarity(dim=1, eps=1e-6)
        else:
